...
#     try:
#         db, cursor = get_db_cursor()
#         # Assuming 'users' is your user table and it has 'id' and 'name' columns
#         query = '''
#             SELECT users.id, users.name 
#             FROM users 
#             INNER JOIN requests ON users.id = requests.user1_id 
#             WHERE requests.user2_id = %s
#         '''
#         cursor.execute(query, (user_id,))
#         results = cursor.fetchall()

#         users_info = [{'id': row[0], 'name': row[1]} for row in results]
...
@app.route('/api/posts/<int:post_id>/delete_post', methods=['DELETE'])
def delete_post(post_id):
    try:
            ...
            cursor.execute('DELETE FROM user_likes WHERE post_id = %s', (post_id,))
            connection.commit()
