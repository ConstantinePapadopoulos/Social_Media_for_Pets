  ...
  Future<void> fetchFriends() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        print(
            'Failed to load user friends. Status code: ${response.statusCode}');
      }
    } catch (e) {
  ...
  Future<void> deleteFriendship(int friendId) async {
    try {
      final response = await http.delete(
        Uri.parse(
            'http://10.0.2.2:5000/api/delete_friendships/${UserAuth.userId}/$friendId'),
      );
      ...
      if (response.statusCode == 204) {
        ...
        print(
            'Failed to delete friendship. Status code: ${response.statusCode}');
      }
    } catch (e) {
  ...
  Future<bool> showDeleteConfirmationDialog(
    ...
    return await showDialog(
          ...
          builder: (BuildContext context) {
            return AlertDialog(
              title: Text('Διαγραφή φίλου'),
              content: Text('Θέλετε σίγουρα να διαγράψετε αυτόν τον φίλο;'),
              actions: [
                TextButton(
                  onPressed: () {
                    Navigator.of(context)
                        .pop(true); // Return true for confirmed deletion
                  },
                  child: Text('Ναι'),
                ),
                TextButton(
